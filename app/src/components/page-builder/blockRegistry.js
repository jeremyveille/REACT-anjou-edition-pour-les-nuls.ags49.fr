...
export const getDefaultHomepageBlocks = () => {
  ...
  const featuredGridSection = createBlock('section', {
    classes: 'py-4 bg-slate-50',
    style: { paddingTop: '2rem', paddingBottom: '2rem' }
  });
  const featuredGridContainer = createBlock('container', { fluid: false });
  const featuredGridHeading = createBlock('heading', {
    content: 'À la une : Flipbooks Interactifs',
    level: 'h3',
    classes: 'section-title mb-3'
  });
  const featuredGridWidget = createBlock('flipbookFeatured', {
    title: 'À la une : Flipbooks Interactifs',
    mode: 'grid',
    items: flipbooksData.map(fb => ({
      id: fb.id,
  ...
  featuredGridContainer.children.push(featuredGridHeading, featuredGridWidget);
  featuredGridSection.children.push(featuredGridContainer);

  const poemsSection = createBlock('section', {
    classes: 'py-4 bg-white',
    style: { paddingTop: '2rem', paddingBottom: '2rem' }
  });
  const poemsContainer = createBlock('container', { fluid: false });
  const poemsHeading = createBlock('heading', {
    content: 'Poésies et Fables Phares',
    level: 'h3',
    classes: 'section-title mb-3'
  });
  const poemsWidget = createBlock('featuredPoems', {
    title: 'Poésies et Fables Phares',
    poems: [
      {
        id: 'poem_1',
  ...
  poemsContainer.children.push(poemsHeading, poemsWidget);
  poemsSection.children.push(poemsContainer);

  return [homeFlipbookSection, featuredGridSection, poemsSection];
};
