 ...
 */
export default function moveElement(elements, activeId, overId, isNew = false) {
  let elementToInsert;
  let updatedTree = [...elements];

  if (isNew) {
    // activeId is the widget type, e.g. "widget-section", "widget-text", etc.
    const type = activeId.replace('widget-', '');
    elementToInsert = createDefaultElement(type);
  } else {
  ...
  if (overId === 'canvas-root' || !overId) {
    ...
    if (elementToInsert.type === 'section') {
      ...
      row.children[0].children.push(elementToInsert);
        ...
        return list.map(el => {
          if (el.id === overId) {
            ...
            if (sectionChildren.length === 0) {
              const row = createDefaultElement('row-1');
              row.children[0].children.push(elementToInsert);
              sectionChildren.push(row);
            } else {
              // Try to put it in the last column of the last row
              const lastRowIdx = sectionChildren.length - 1;
              const lastRow = { ...sectionChildren[lastRowIdx] };
              if (lastRow && lastRow.type === 'row' && lastRow.children.length > 0) {
                const lastColIdx = lastRow.children.length - 1;
                const lastCol = { ...lastRow.children[lastColIdx] };
                lastCol.children = [...lastCol.children, elementToInsert];
                
                const newRowChildren = [...lastRow.children];
                newRowChildren[lastColIdx] = lastCol;
                lastRow.children = newRowChildren;
                
                sectionChildren[lastRowIdx] = lastRow;
              } else {
                const row = createDefaultElement('row-1');
                row.children[0].children.push(elementToInsert);
                sectionChildren.push(row);
